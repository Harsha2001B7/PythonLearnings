...
def create_testimonial(*, db: Session = Depends(get_db), testimonial_in: TestimonialBase):
    return testimonial_repo.create(db, obj_in=testimonial_in)

@router.put("/{id}", response_model=Testimonial)
def update_testimonial(*, db: Session = Depends(get_db), id: int, testimonial_in: TestimonialBase):
    testimonial = testimonial_repo.get(db, id=id)
    if not testimonial:
        raise HTTPException(status_code=404, detail="Testimonial not found")
...
def delete_testimonial(*, db: Session = Depends(get_db), id: int):
    testimonial = testimonial_repo.get(db, id=id)
    if not testimonial:
        raise HTTPException(status_code=404, detail="Testimonial not found")

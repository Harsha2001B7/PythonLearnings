...
def create_faq(*, db: Session = Depends(get_db), faq_in: FAQBase):
    return faq_repo.create(db, obj_in=faq_in)

@router.put("/{id}", response_model=FAQ)
def update_faq(*, db: Session = Depends(get_db), id: int, faq_in: FAQBase):
    faq = faq_repo.get(db, id=id)
    if not faq:
        raise HTTPException(status_code=404, detail="FAQ not found")
...
def delete_faq(*, db: Session = Depends(get_db), id: int):
    faq = faq_repo.get(db, id=id)
    if not faq:
        raise HTTPException(status_code=404, detail="FAQ not found")

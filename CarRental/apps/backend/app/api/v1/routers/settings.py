...
def create_setting(*, db: Session = Depends(get_db), setting_in: SiteSettingBase):
    return site_setting_repo.create(db, obj_in=setting_in)

@router.put("/{id}", response_model=SiteSetting)
def update_setting(*, db: Session = Depends(get_db), id: int, setting_in: SiteSettingBase):
...
def delete_setting(*, db: Session = Depends(get_db), id: int):
    setting = site_setting_repo.get(db, id=id)
    if not setting:
        raise HTTPException(status_code=404, detail="Setting not found")
